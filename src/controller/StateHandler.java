...
public class StateHandler {
    ...
    public GameState changeState(Game game, String state){
        ...
        switch (state) {
            case "SELECTION": SelectionState selectionState = new SelectionState();
                              selectionState.setState(game, context);
                              return selectionState;
            case "GAMEOVER":  GameOverState gameoverState = new GameOverState();
                              gameoverState.setState(game, context);
                              return gameoverState;
            case "PAUSE":     PauseState pauseState = new PauseState();
                              pauseState.setState(game, context);
                              return pauseState;
            case "SUCCESS":   SuccessState successState = new SuccessState();
                              successState.setState(game, context);
                              return successState;
            case "PLAY":      PlayState playState = new PlayState();
                              playState.setState(game, context);
                              return playState;
            default : SelectionState sState = new SelectionState();
